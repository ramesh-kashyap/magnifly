...
class DepositController extends Controller
{
    ...
    public function deposits(Request $request)
    {
        ...
        if($search==1)
        {
        $notes = Investment::where('status','Active')->where('active_from','Admin')->orderBy('id', 'DESC');     
        }
        else
        {
         $notes = Investment::where('status','Active')->whereNotIn('active_from',['Admin','Deposit Bonus'])->orderBy('id', 'DESC');         
        }
    ...
    $notes = $notes->paginate($limit)
        ...
        $this->data['deposit_list'] =  $notes;
        $this->data['search'] = $search;
        $this->data['page'] = 'admin.deposit.deposits';
        return $this->admin_dashboard();
    }
