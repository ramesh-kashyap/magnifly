...
$investment = Investment::select(
    ...
    'created_at as date',
    'active_from' // include this column so it's available in the map
)
...
->get()
->map(function ($item) {
    $item->type = $item->active_from === 'Deposit Bonus' ? 'Deposit Bonus' : 'investment';
...
->where('user_id', $userId)
->whereIn('status', ['Approved', 'Pending']) // ðŸ‘ˆ Add this line
->orderBy('created_at', 'desc')
->get()
->map(function ($item) {
    $item->type = 'withdraw';
...
->where('user_id', $userId)
->where('c_status', -1) // ✅ Add this line to check c_status = -1
->orderBy('created_at', 'desc')
->get()
->map(function ($item) {
    $item->type = 'contract';
...
public function updatePopupCheck(Request $request)
{
    $user = auth()->user();
    $user->popup_check = true;
    $user->save();
